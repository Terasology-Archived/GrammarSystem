...
public class SplitRule extends ComplexRule {
    ...
    private Shape getInnerElement(Shape shape) {
        ...
        int deltaX = (dimension.x > 0) ? dimension.x - 2 : dimension.x + 2;
        int deltaY = (dimension.y > 0) ? dimension.y - 2 : dimension.y + 2;
        int deltaZ = (dimension.z > 0) ? dimension.z - 2 : dimension.z + 2;

        shape.setDimension(deltaX, deltaY, deltaZ);
        return shape;
    }
    ...
    private List<Shape> getWallElements(Shape shape) {
        ...
        s.setMatrix(new Matrix4i(matrix));
